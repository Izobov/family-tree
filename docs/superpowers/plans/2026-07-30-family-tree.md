...
- **Иконки — только MDI через `$lib/icons`.** Никаких эмодзи и юникодных глифов в интерфейсе: они рисуются разным весом и по-разному на разных платформах. В Svelte — компонент `<Icon name="..." />`, в карточке дерева (её вставляет family-chart строкой вне Svelte) — функция `svgIcon(name)`. Выбор конкретной иконки живёт только в `src/lib/icons.ts`, компоненты просят роль, а не начертание. `label` у иконки задаём **только** когда она единственное содержимое кнопки; рядом с видимой подписью иконка `aria-hidden`, иначе скринридер прочитает подпись дважды.
